...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // Bypasses wrangler r2 object put's 300 MiB cap (e.g. multi-GB Wan-2.1 weights).
...
// One-shot streaming mirror: fetch a HuggingFace file and write it to R2 via
// the env.CONTENT binding. Workers' fetch->R2 path has no 300 MiB cap (unlike
// `wrangler r2 object put` from the host machine). Streaming happens entirely
// on Cloudflare's network — bytes never come back to the caller.
//
// Used to mirror the DreamID-V + Wan-2.1 + DWPose weights into r2://.../models/
// for the HARDEN_WEIGHTS_R2 cold-boot fast-path (the pod fetches them via the
// worker proxy, no HuggingFace at boot).
//
// Body (JSON): {
//   hf_repo:     "Wan-AI/Wan2.1-T2V-1.3B"               (required)
//   hf_filename: "diffusion_pytorch_model.safetensors"  (required)
//   hf_token:    "hf_..."                               (optional; only for gated repos)
//   r2_key:      "models/wan2.1/diffusion_pytorch_model.safetensors" (required, must start with "models/")
// }
//
